(function(){
  ...
  function applySignalState(svg, config) {
    ...
    const applyTheatre = (sideKey, text) => {
      ...
      node.setAttribute('font-family', '"LEDDotMatrix", LEDDotMatrixRegular');
    };
    ...
    if (plateVisible) {
      ...
      if (textNode) {
        ...
        if (plateNode) {
          ...
          const absoluteCenterX = plateX + (plateWidth / 2);

          const transform = textNode.getAttribute('transform') || '';
          const match = transform.match(/translate\(([-\d.]+)[ ,]([-\d.]+)\)/);
          const textTranslateX = match ? parseFloat(match[1]) : 0;
          centeredX = absoluteCenterX - textTranslateX;
        }
