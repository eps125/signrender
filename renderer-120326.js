(function(){
  ...
  function fitTextNode(node, options = {}) {
    if (!node || !node.ownerSVGElement) return;
    const { maxWidth = Infinity, maxHeight = Infinity, centerX = null, centerY = null } = options;
    let bbox;
    try {
      bbox = node.getBBox();
    } catch (error) {
    ...
    if (centerX != null || centerY != null) {
      node.setAttribute('transform', `translate(${centerX != null ? centerX : 0},${centerY != null ? centerY : 0}) scale(${scale}) translate(${-(centerX != null ? centerX : 0)},${-(centerY != null ? centerY : 0)})`);
    } else {
      node.setAttribute('transform', `scale(${scale})`);
    }
  }

  function normalizeCsmConfig(config = {}) {
    const rawValue = (config.value || '')
      ...
      .replace(/\s+/g, '')
      .trim();
    const value = /^(S|\d{1,2}X?|\d{1,2}[\-/]\d{1,2}X?)$/.test(rawValue) ? rawValue : '';
    const traction = Array.isArray(config.traction)
  ...
  function parseCsmMarker(value) {
    ...
    const isPrecision = upper.endsWith('X');
    const base = isPrecision ? upper.slice(0, -1) : upper;
    ...
    const boardWidth = (left.length > 1 || right.length > 1 || isPrecision) ? 640 : 502;
    return {
      kind: 'range',
      separator,
      isPrecision,
      left,
      right,
      display: `${left} ${separator} ${right}${isPrecision ? 'X' : ''}`,
      boardWidth
    };
  }

  function finalizeCsmLayout(svg) {
    Array.from(svg.querySelectorAll('[data-fit-max-width]')).forEach(node => {
      fitTextNode(node, {
        maxWidth: Number(node.getAttribute('data-fit-max-width')),
        maxHeight: Number(node.getAttribute('data-fit-max-height')),
        centerX: Number(node.getAttribute('data-fit-center-x')),
        centerY: Number(node.getAttribute('data-fit-center-y'))
      });
    });
  }

  function appendFittedText(svg, attrs, textContent) {
    const text = createSvgEl('text', attrs);
    text.textContent = textContent;
    svg.appendChild(text);
    return text;
  }

  async function renderCSM(container, config = {}, scale = 1) {
    const normalized = normalizeCsmConfig(config);
    if (!normalized.value) {
      container.innerHTML = '';
      return;
    }

    const marker = parseCsmMarker(normalized.value);
    const boardWidth = marker.boardWidth;
    const boardHeight = 250;
    const classBoxWidth = 250;
    const classBoxHeight = 30;
    const classGap = 6;
    const classStackHeight = normalized.traction.length
      ? (normalized.traction.length * classBoxHeight) + ((normalized.traction.length - 1) * classGap) + classGap
      : 0;
    const totalWidth = Math.max(boardWidth, classBoxWidth);
    ...
    if (marker.kind === 'single') {
      ...
      const sideGap = 42;
      const separatorX = boardX + (boardWidth / 2);
      const leftAreaLeft = boardX + leftMargin;
      const rightAreaRight = boardX + boardWidth - rightMargin;
      const leftAreaRight = separatorX - sideGap;
      const rightAreaLeft = separatorX + sideGap;
      const centerY = boardY + 105;
      const numberMaxHeight = 150;

      appendFittedText(svg, {
        x: (leftAreaLeft + leftAreaRight) / 2,
        y: centerY + 4,
        fill: '#fff',
        'font-family': 'BRLight',
        'font-size': 200,
      ...
      appendFittedText(svg, {
        x: (rightAreaLeft + rightAreaRight) / 2,
        y: centerY + 4,
        fill: '#fff',
        'font-family': 'BRLight',
        'font-size': 200,
        'text-anchor': 'middle',
        'dominant-baseline': 'middle'
      }, `${marker.right}${marker.isPrecision ? 'X' : ''}`);
    }
  ...
  function normalizeSignalConfig(config = {}) {
    ...
    const normalizeJiValue = (value) => ['None', 'Blank', 'White', 'Green', 'Yellow', 'Red'].includes(value) ? value : 'None';
  ...
  function applySignalState(svg, config) {
    ...
    const jiValues = Array.isArray(config.ji) ? config.ji : [];

    const anyJiActive = jiValues.some(v =>
